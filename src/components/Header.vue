...
import { computed } from 'vue'
    ...
    const totalPay = computed(() => {
        return props.cart.reduce( (total, product) => total = total + (product.stock * product.precio ), 0)
    })

    defineEmits(['decrementStock','incrementStock', 'addToCart', 'deleteProduct', 'emptyCart'])
</script>
<template>
                                        ...
                                        >
                                            ...
                                            <td class="flex align-items-start gap-4">
                                                <button
                                                    ...
                                                    @click="$emit('decrement-stock', product.id)"
                                                >
                                                    -
                                                </button>
                                                    {{ product.stock }}
                                                <button
                                                    type="button"
                                                    class="btn btn-dark"
                                                    @click="$emit('increment-stock', product.id)"
                                                >
                                                    +
                                                </button>
                                            </td>
